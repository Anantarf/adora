...
export const metadata: Metadata = {
  ...
  description: "Pusat pembinaan bola basket profesional di Indonesia. Membentuk generasi atlet berkarakter, berprestasi, dan berintegritas.",
  openGraph: {
...
const PROGRAMS = [
  {
    label: "KU‑10",
    ages: "10 – 12 Tahun",
    desc: "Fondasi skill dasar: koordinasi bola, ball-handling, dan kecintaan terhadap olahraga basket sejak dini.",
    icon: "🌱",
  },
  {
    label: "KU‑15",
    ages: "13 – 15 Tahun",
    desc: "Pengembangan taktik, fisik, dan mental kompetisi untuk pemain yang sedang berkembang pesat.",
    icon: "⚡",
  },
  {
    label: "KU‑18",
    ages: "16 – 18 Tahun",
    desc: "Persiapan penuh menuju kompetisi tingkat provinsi dan nasional dengan standar pelatihan profesional.",
    icon: "🏆",
  },
];
...
export default async function LandingPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0d0d0d] text-foreground relative overflow-hidden">
      ...
      {/* ── Hero ── */}
      <section className="relative z-10 min-h-[calc(100dvh-4rem)] flex flex-col items-center justify-center text-center px-4 py-20">
        ...
        <p className="text-sm md:text-base text-white/60 max-w-xl mx-auto mb-10 leading-relaxed">
          Membentuk generasi atlet basket berkarakter, berprestasi,
      ...
      <div className="relative z-10 w-full border-y border-white/8 bg-primary/[0.04]">
        ...
        <dl className="container mx-auto px-4 py-8 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
        ...
        <section id="program" className="container mx-auto px-4 py-24">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {PROGRAMS.map(({ label, ages, desc, icon }) => (
              <div
                key={label}
                className="group bg-white/5 border border-white/10 hover:border-primary/40 rounded-2xl p-8 text-center transition-all duration-300 hover:bg-white/[0.08]"
              >
      ...
      <section id="galeri" className="container mx-auto px-4 py-24 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-4 max-w-4xl mx-auto" role="list" aria-label="Galeri foto kegiatan">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              role="listitem"
              className="relative bg-[#111113] border border-primary/20 rounded-2xl h-56 overflow-hidden"
            >

...
import { CalendarDays, MapPin, UserRound, VenusAndMars } from "lucide-react";
...
export function ParentPlayerHero({ player }: { player: FamilyPlayer }) {
  return (
    <Card className="border-border/50 bg-card shadow-sm">
      <CardContent className="grid gap-5 p-5 md:grid-cols-[140px_minmax(0,1fr)] md:items-center">
        <div className="flex justify-center md:justify-start">
          <div className="flex h-36 w-28 items-center justify-center overflow-hidden rounded-2xl border border-border/60 bg-muted/30">
            {player.photoUrl ? (
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <h2 className="text-2xl font-heading uppercase tracking-wide text-foreground md:text-3xl">
              {player.name}
            </h2>
          </div>
          ...
          <div className="flex items-start gap-2 rounded-xl border border-border/50 bg-background/40 px-3 py-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm font-medium text-foreground">{player.group?.name || "-"}</p>
            </div>
          </div>
        </div>
      </CardContent>

...
const getBadgeLines = (label: string): string[] => (label === "Agenda Lainnya" ? ["Agenda", "Lainnya"] : [label.slice(0, 10)]);
    ...
    <>
      ...
      <FullCalendar
        ...
        eventContent={(arg) => {
          ...
          return (
            <div className="flex items-center gap-1.5 p-[2px] pr-2.5 rounded-full border border-white/20 bg-white/10 backdrop-blur-sm w-fit max-w-[98%] overflow-hidden hover:bg-white/20 transition-all cursor-pointer group shadow-sm my-1 mx-auto">
              <div className="flex items-center justify-center size-[16px] flex-shrink-0 rounded-full text-white shadow-sm" style={{ backgroundColor: cfg.color }}>
                <Icon className="size-[9px]" strokeWidth={2.5} />
              </div>
              <span className={`text-[7px] sm:text-[7.5px] font-medium tracking-wider uppercase text-white group-hover:text-white transition-colors leading-none whitespace-nowrap truncate ${isMultiLine ? "flex flex-col gap-[2px]" : ""}`}>
                {isMultiLine ? (
                  lines.map((line) => (

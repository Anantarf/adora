    ...
    <div className="flex flex-col gap-6">
      ...
      {months.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 gap-3 rounded-2xl border border-dashed border-border/50 text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground">{searchQuery ? "Hasil tidak ditemukan" : "Belum ada agenda yang tercatat"}</p>
          <p className="text-xs text-muted-foreground/75 mt-1">{searchQuery ? "Coba gunakan kata kunci pencarian yang berbeda." : "Silakan buat agenda baru di menu Jadwal."}</p>
        </div>
      ) : (
                ...
                {groupedByMonth[monthKey].map((event) => {
                  ...
                  return (
                    <Button
                      ...
                      className="w-full h-auto p-3 sm:p-3.5 justify-start hover:bg-muted/50 transition-all rounded-xl border border-border/50 group"
                    ...
                    >
                      <div className="grid w-full grid-cols-1 sm:grid-cols-[64px_minmax(0,1fr)_auto] gap-3 items-center text-left">
                        <div className="hidden sm:flex flex-col items-center justify-center rounded-lg px-2 py-2 border transition-colors" style={{ backgroundColor: `${cfg.color}15`, borderColor: `${cfg.color}30` }}>
                          <span className="text-lg leading-none font-black" style={{ color: cfg.color }}>
                            {format(eventDate, "dd", { locale: idLocale })}
                          </span>
                          <span className="text-micro" style={{ color: cfg.color }}>
                            {format(eventDate, "MMM", { locale: idLocale })}
                          </span>
                        </div>
                        ...
                        <div className="min-w-0 space-y-1">
                          <div className="flex flex-col gap-0.5">
                            <span
                              className="inline-flex w-fit px-1.5 py-0.5 rounded text-[10px] font-black uppercase tracking-[0.15em] border leading-none"

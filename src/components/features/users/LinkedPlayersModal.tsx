    ...
    <Dialog open={!!parentId} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md bg-card border-border/50 p-0 overflow-hidden flex flex-col max-h-[85vh]">
        <DialogHeader className="px-5 pt-5 pb-4 shrink-0 border-b border-border/50">
          <DialogTitle className="font-heading text-lg uppercase tracking-wide text-foreground">
            {isAdding ? "Hubungkan Pemain" : "Pemain Terhubung"}
          </DialogTitle>
          <DialogDescription className="text-xs text-muted-foreground mt-1.5">
            {isAdding 
              ? "Cari dan pilih pemain untuk ditautkan ke akun ini."
              : (parentName ? `Daftar pemain yang terhubung ke akun ${parentName}.` : "Daftar pemain yang terhubung ke akun ini.")}
          </DialogDescription>
        </DialogHeader>
              ...
              {isAvailableLoading ? (
                ...
                <div className="flex flex-col items-center justify-center gap-2 py-10 rounded-xl border border-dashed border-border/50">
                  ...
                  <p className="text-sm font-medium text-muted-foreground text-center">
                    {searchTerm ? "Tidak Ditemukan" : "Tidak Ada Pemain"}
                  </p>
                  <p className="text-xs text-muted-foreground/60 text-center px-4">
                    {searchTerm ? "Coba gunakan kata kunci lain." : "Semua pemain sudah terhubung ke akun orang tua."}
                  </p>
                </div>
              ) : (
                  ...
                  {filteredAvailablePlayers.map((p) => (
                    <li key={p.id} className="flex items-center justify-between p-3 rounded-xl border border-border/50 bg-background/40 hover:border-primary/30 transition-colors">
                      <div className="flex flex-col min-w-0 pr-3">
                        ...
                        <span className="text-micro text-muted-foreground/60 truncate mt-0.5">
                          {p.group?.name ?? "Belum ada kelompok"}
                        </span>
                      </div>
                      <Button
                        ...
                        disabled={isLinking}
                        size="sm"
                        className="h-8 px-3 text-micro rounded-lg shrink-0 shadow-none"
                      >
          ...
          <div className="flex flex-col flex-1 min-h-0">
            <div className="p-4 shrink-0 border-b border-border/50 bg-background/30">
              <Button
                ...
                className="w-full h-10 border border-dashed border-primary/40 bg-primary/5 text-primary hover:bg-primary/10 hover:border-primary transition-all font-semibold shadow-none"
              >
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center gap-2 py-8 rounded-xl border border-dashed border-border/50">
                  ...
                  <p className="text-xs text-muted-foreground/60 text-center px-4">Akun ini belum terhubung dengan data anak mana pun.</p>
                </div>
              ) : (
                <ul className="space-y-2">
                  {players.map((p) => (
